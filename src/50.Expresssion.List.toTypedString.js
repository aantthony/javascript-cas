	...
	"x-shader/x-fragment":function(o, x){
		...
		switch(o){
			...
			case ",":
				...
				return {s:S_(x[0])+o+S_(x[1]), t: glsl.float, p: p};
			case "*":
			case "/":
				return {s:S_(x[0])+o+S_(x[1]), t: glsl.float, p: p};
			case "_":
			...
			case "^":
				...
				return {s:"pow("+x[0].s+","+x[1].s+")",t:glsl.float, p: p};
			case undefined:
				if(x[0].t===glsl.function){
					return {s:x[0].s+"("+x[1].s+")",t:glsl.float, p: p};
				}else{
					//this is ugly:
					p=language.precedence("*");
					return {s:S_(x[0])+"*"+S_(x[1]),t:glsl.float, p: p};
				}
			case "#":
				throw("Anonymous functions not supported.");
			case "√":
				return {s:"sqrt("+x[0].s+")",t:glsl.float, p: p};
			case "!":
				//requirements....
				return {s:"factorial("+x[0].s+")",t:glsl.float, p: p};
			case "%":
				return {s: "mod("+x[0].s+","+x[1].s+")",t:glsl.float, p:p};
			case "&":
			case "|":
			//case "%":
			case "~":
