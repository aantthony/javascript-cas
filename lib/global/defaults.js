...
module.exports.attach = function (global) {
    ...
    function AbstractDiscreteGroup(n, operator) {
        if (operator) throw new Error('There cannot be an operator!');
        var elements = [];
        for(var i = 0; i < n; i++) {
            elements.push(new Element());
        }
        function Element(n) {
            if (n !== undefined) return elements[n];
            this.n = n;
        }
    ...
    function ModuloGroup(n, operator) {
        ...
        var Element = new AbstractDiscreteGroup(n);
    ...
    function E(n) {
        if (typeof n === 'number') {
            ...
            if (x instanceof Expression.NumericalReal) {
                return x.value > 0 ? 0 : 2;
            }
            if (x instanceof Expression.List.Real) {
                return x.map(E.create);
            }
        }
    }
