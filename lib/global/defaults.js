...
module.exports.attach = function (global) {
    ...
    util.inherits(PowerGroup, FiniteSet);
    function PowerGroup(Group, operator) {
        FiniteSet.call(this);

        operator = operator || 'default';

        function Element(_, p) {
            this.n = n;
        }
    ...
    var PE = new PowerGroup(E, '*');

    // map expressions onto PE
    PE.fromExpression = function (x) {
        if (x instanceof Expression.NumericalReal) {
            return new PE(x.value > 0 ? [E.elements[0]] : [E.elements[2]]);
        }
        if (x instanceof Expression.List.Real) {
            return x.map(PE.fromExpression);
        }
