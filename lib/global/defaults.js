...
module.exports.attach = function (global) {
    ...
    var E = new ModuloGroup(4, '*');

    var atLeastReal = new MultiValue([E.elements[0], E.elements[2]])

    var atLeastImaginary = new MultiValue([E.elements[1], E.elements[3]])


    function direction(A, B, mapping) {

        if (mapping(A).equals(B)) {
            return A;
        }

        if (A.subset(atLeastReal) && B.subset(atLeastReal)) {
            return atLeastReal;
        }

        if (A.subset(atLeastImaginary) && B.subset(atLeastImaginary)) {
            return atLeastImaginary;
        }
    }

    // map expressions onto PE
    function PE (x) {
        if (x instanceof Expression.NumericalReal) {
            return x.value > 0 ? new MultiValue([E.elements[0]]) : new MultiValue([E.elements[2]]);
        }
        if (x instanceof Expression.List) {
            var A = PE(x[0]);
            var B = PE(x[1]);
            ...
            switch(x.operator) {
                case '*':
                    return A['+'](B);
                case '/':
                    return direction(A, B, function (A) {
                        return A.default(E.elements[2]);
                    });
                case '+':
                    return direction(A, B, function (A) {
                        return A;
        ...
        if (x instanceof Expression.List.ComplexCartesian) {
            return new PE([E.elements[0], E.elements[1], E.elements[2], E.elements[3]]);
        }
        if (x instanceof Expression.List.ComplexPolar) {
        ...
        throw new Error('Mapping to PE from ' + x.constructor.name + ' not defined');
